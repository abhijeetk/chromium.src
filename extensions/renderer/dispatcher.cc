...
v8::Local<v8::Value> GetOrCreateChrome(ScriptContext* context, bool hidden, const char* name = nullptr) {
  ...
  if (!hidden) {
    ...
    v8::Local<v8::String> privates_key(
       v8::String::NewFromUtf8(context->isolate(), "privates"));
    
    v8::Local<v8::Value> privates(global->GetHiddenValue(privates_key));
    if (privates.IsEmpty()) {
      privates = v8::Object::New(context->isolate());
      global->SetHiddenValue(privates_key, privates);
    }
